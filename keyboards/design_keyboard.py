...
def carousel_callback(service):
    keyboard = [{"action": {"type": "callback", "label": f"{service}", "payload": json.dumps(f"{service}")}, "color": "positive"}, {"action": {"type": "callback", "label": f"Назад к категориям", f"payload": json.dumps("back_to_categorie")}, "color": "negative"}]
    return keyboard
def nextPage(page):
    keyboard = [{"action": {"type": "callback", "label": f"Следующая страница", f"payload": json.dumps(page)}, "color": "positive"}, {"action": {"type": "callback", "label": f"Назад в меню", f"payload": json.dumps("back_to_categorie")}, "color": "negative"}]
    return keyboard
def lastPage(page):
    keyboard = [{"action": {"type": "callback", "label": f"Предыдущая страница", f"payload": json.dumps(page)}, "color": "positive"}, {"action": {"type": "callback", "label": f"Назад в меню", f"payload": json.dumps("back_to_categorie")}, "color": "negative"}]
    return keyboard
